...
- The wazero execution backend is the `engine` YAML field (`auto` | `interpreter` | `compiler`), mapped in `Engine.runtimeConfig()`. It defaults to `auto`, so on amd64/arm64 assume measurements were taken compiled — the interpreter is at least an order of magnitude slower, which is why `auto` won the default. `compiler` is deliberately not the default: `NewRuntimeConfigCompiler()` panics inside wazero on any arch but amd64/arm64, and a panic at component-create time isn't something `Config.Validate` can turn into a YAML error — `auto` is the value that probes and falls back.
