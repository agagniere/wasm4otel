...
- `alloc(size: u32) -> u32`, `free(ptr: u32, size: u32)` — required alongside `consume_*` for processor/exporter mode. The host calls `alloc` to reserve a region in the guest's linear memory, writes the OTLP payload into it, invokes the `consume_*` export, then calls `free`. Returning 0 from `alloc` signals failure; the host treats 0 as a non-trapping skip and does not call `free`.

The role each plugin plays is decided by which factory the operator registers in the OTel collector YAML — `receivers:`, `processors:`, or `exporters:` — not by the plugin itself. The factory validates at create-time that the plugin exports the symbols its role needs (`start` for receiver; `consume_logs`/`alloc`/`free` for processor/exporter on the logs signal).
...
- After any guest trap (`alloc`, `consume_*`, `free`, `stop`), `Component.broken` latches and further guest entries fail closed. The collector will see a stream of errors; we don't try to recover the instance.
