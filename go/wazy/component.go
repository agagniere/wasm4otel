...
import (
	...
	wazy_api "github.com/samyfodil/wazy/api"
	wazy_sys "github.com/samyfodil/wazy/sys"
...
type Component struct {
	...
	setup    wazy_api.Function
	start    wazy_api.Function
	receive  wazy_api.Function
	shutdown wazy_api.Function
	...
	processLogs    wazy_api.Function
	processMetrics wazy_api.Function
	processTraces  wazy_api.Function
	exportLogs     wazy_api.Function
	exportMetrics  wazy_api.Function
	exportTraces   wazy_api.Function

	alloc wazy_api.Function
	free  wazy_api.Function
	...
	// callMu serializes every Call() into the wasm instance. A wazy
	// module is single-occupant; without this two pipeline
	// goroutines could race on a shared linear memory.
	callMu std_sync.Mutex
	// broken latches once any guest Call() traps. The runtime leaves the
...
func newRuntime(context std_context.Context, mode RuntimeMode) wazy.Runtime {
	runtime := wazy.NewRuntimeWithConfig(context, mode.runtimeConfig())
	wasi_snapshot_preview1.MustInstantiate(context, runtime)
	return runtime
}

// runtimeConfig maps the configured mode onto wazy's runtime
// constructors — inherited from wazero unchanged, so the field means
// the same thing in both hosts. RuntimeModeAuto — the default —
// defers to the runtime's own probe: does this GOOS/GOARCH have a
// backend, and will the kernel hand out executable pages.
// RuntimeModeCompiled is the unconditional form: on a platform with
// no backend it panics inside the runtime, which is why the default
// asks for native speed through the probe rather than by naming the
// compiler outright.
func (mode RuntimeMode) runtimeConfig() wazy.RuntimeConfig {
	switch mode {
	case RuntimeModeCompiled:
		return wazy.NewRuntimeConfigCompiler()
...
func (self *Component) ExposeFunctionsToGuest() error {
	...
	wazy.HostFunc2(builder.NewFunctionBuilder(), self.outboundMetrics).Export("push_metrics")
	wazy.HostFunc2(builder.NewFunctionBuilder(), self.outboundTraces).Export("push_traces")
	wazy.HostFunc1(builder.NewFunctionBuilder(), self.interruptibleSleepMs).Export("interruptible_sleep_ms")
	wazy.HostFunc2(builder.NewFunctionBuilder(), self.getConfig).Export("get_config")
...
func (self *Component) getConfig(_ std_context.Context, module wazy_api.Module, ptr uint32, size uint32) uint32 {
...
func (self *Component) Shutdown(context std_context.Context) error {
	...
	// Release the instance and everything the runtime compiled for it.
	// Safe even after a trap: closing is the one operation a poisoned
	// instance still owes us.
	if err := self.runtime.Close(context); err != nil {
		self.guestLogger.Warnw("runtime close returned with error", "error", err)
	}
	return nil
}
...
func (self *Component) invoke(ctx std_context.Context, fn wazy_api.Function, args ...uint64) ([]uint64, error) {
	self.callMu.Lock()
...
//
// The unused api.Module parameter is what wazy's HostFunc1 expects; the
// wazero host omits it.
func (self *Component) interruptibleSleepMs(_ std_context.Context, _ wazy_api.Module, ms uint32) uint32 {
	select {
...
func (self *Component) logToZap(_ std_context.Context, module wazy_api.Module, level int32, offset uint32, size uint32) {
	buffer, ok := module.Memory().Read(offset, size)
	if !ok {
		self.guestLogger.Errorf("Unable to read (%d, %d) from memory", offset, size)
		return
...
func (self *Component) LoadPlugin() error {
	...
	// Opt out of the runtime's deterministic defaults: without these, a
	...
	if err != nil {
		// A guest that called proc_exit during instantiation reports it
		// as an ExitError; anything else is a malformed or unlinkable
		// module. Both are the operator's problem and both travel out
		// through Load as a plain error — a library has no business
		// panicking the collector over a bad plugin file.
		var exitErr *wazy_sys.ExitError
		if std_errors.As(err, &exitErr) && exitErr.ExitCode() != 0 {
			return std_fmt.Errorf("wasm4otel %s: plugin %s exited with code %d during instantiation",
				self.mode, self.config.Path, exitErr.ExitCode())
		}
		return std_fmt.Errorf("wasm4otel %s: instantiate %s: %w", self.mode, self.config.Path, err)
	}
...
func (self *Component) logsExport() (wazy_api.Function, string) {
...
func (self *Component) metricsExport() (wazy_api.Function, string) {
	if self.mode == ModeExporter {
...
func (self *Component) tracesExport() (wazy_api.Function, string) {
	if self.mode == ModeExporter {
...
func (self *Component) validateSignalExport(fn wazy_api.Function, name string, signal string) error {
	if fn == nil {
...
func (self *Component) deliver(ctx std_context.Context, fn wazy_api.Function, payload []byte, export string) error {
	if len(payload) == 0 {
		return nil
	}
	self.callMu.Lock()
...
func (self *Component) deliverLocked(ctx std_context.Context, fn wazy_api.Function, payload []byte, export string) error {
...
// — wazy modules are single-occupant and a partial sequence must
