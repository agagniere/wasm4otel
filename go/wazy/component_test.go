...
// nopReceiverSettings are what the receiver factory would hand
// LoadReceiver: an ID and a telemetry stack for receiverhelper's
// ObsReport to report into. Going through LoadReceiver rather than
...
// back out of ProcessLogs means alloc, memory write, the batch export,
// the push import and free all agree on the ABI.
//
// Processor mode returns the batch rather than forwarding it: for the
// span of the call push_logs appends to the result instead of reaching
// the next consumer, so processorhelper can time the guest alone. The
// downstream hand-off is processorhelper's job, so there is no sink
// here to check.
func TestProcessorRoundTrip(t *std_testing.T) {
	...
	out, err := component.ProcessLogs(std_context.Background(), oneLogRecord("hello"))
	if err != nil {
		t.Fatalf("ProcessLogs: %v", err)
	}

	if got := out.LogRecordCount(); got != 1 {
		t.Fatalf("returned record count = %d, want 1", got)
	}
	body := out.ResourceLogs().At(0).
...
func TestReceiveLoopRunsAndStops(t *std_testing.T) {
	component, err := LoadReceiver(testConfig(fullPlugin), nopReceiverSettings())
	if err != nil {
